...
# ╔═╡ 74229db1-870c-45aa-8d75-1c85a390c3b4
begin
	using ControlSystems, OrdinaryDiffEq, Plots, Pkg, LinearAlgebra, Statistics, PlutoUI
	gr(show=false, size=(500,400)) # Set defaults for plotting
end

# ╔═╡ 548da4c7-3696-457d-9a1a-1d2be7dc07f4
using NLopt, ForwardDiff

# ╔═╡ 6147e0a7-cd0d-4170-b1f8-af6263cac3b8
md"""
# Auto tuning
...
# ╔═╡ e5783b41-f38e-4318-8c85-8ac9c2785a83
begin
s      = Simulator(Ps)
...
# ╔═╡ ea7c6a3e-bda7-4bdf-9562-51bec9c12693
md"""
## Auto tuning
...
# ╔═╡ 6d53dc20-20d8-4f8d-8e34-f2771cf57c27
begin
...
# ╔═╡ 869e841e-3ee0-4266-932f-1259b325a0f7
begin
@info "Starting Optimization"
@time popt = runopt(p, x_tol=1e-6)
y = timedomain(popt)
plot(t,y')
end

# ╔═╡ 0ecff517-8611-4aba-8902-0ee442452e18
md"""
...
begin
	...
	function freqdomain(p)
		C  = Kpid(p[1], p[2], p[3])
		PC = P*C
		S  = feedback(1, PC) # Sensitivity fun
		T  = feedback(PC)    # Comp. Sensitivity fun
		Sw = bodev(S,Ω,unwrap=false)[1] # Freq. domain constraints
		Tw = bodev(T,Ω,unwrap=false)[1] # Freq. domain constraints
		Sw,Tw
	end
	function constraintfun(p)
	    Sw,Tw = freqdomain(p)
	    [maximum(Sw); maximum(Tw)]
	end
	
	g_cfg = ForwardDiff.JacobianConfig(constraintfun, p)
end

# ╔═╡ 0aec68aa-7126-410b-b8f3-8b1d5bfecdf0
begin
	Ms_gui = @bind Ms Slider(1:0.1:2, default=1.1, show_value=true)
	Mt_gui = @bind Mt Slider(1:0.1:2, default=1.1, show_value=true)
	md"``M_S`` = $Ms_gui  ``M_T`` = $Mt_gui"
end

# ╔═╡ 6b954e81-3582-44e5-adcf-1289d1db3234
begin
	function c(result, p::Vector, grad)
	    if length(grad) > 0
	        grad .= ForwardDiff.jacobian(constraintfun,p,g_cfg)'
	    end
	    result .= constraintfun(p) .- [Ms, Mt]
	end
	
	function runopt2(p; f_tol = 1e-5, x_tol = 1e-3, c_tol = 1e-8)
	    opt = Opt(:LD_SLSQP, 3)
...
# ╔═╡ 24f710ff-a5ba-4790-96ac-5c15cedbc237
begin
	popt2 = runopt2(p, x_tol=1e-6)
	y2 = timedomain(popt2)
	Sw,Tw = freqdomain(popt2)
	plot(t,y2', layout=2, size=(800,400))
	plot!(Ω, [Sw Tw] , lab=["Sw" "Tw"], subplot=2, xscale=:log10, yscale=:log10)
	hline!([Ms], c = 1, l=:dash, subplot=2, lab="Ms")
	hline!([Mt], c = 2, l=:dash, subplot=2, lab="Mt", ylims=(0.01,3))
end

# ╔═╡ a40a824f-e2d6-4487-924f-cf6256386a56
begin
	C2 = Kpid(popt2...)
	gangoffourplot(P,C2, exp10.(LinRange(-1,3,500)), legend=false)
end
...
manifest_format = "2.0"
project_hash = "957df634c16e088af259a3402ca52fec95566269"
