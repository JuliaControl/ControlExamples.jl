...
graph LR
    ...
    V1 -->|ODESystem| MTK2[ModelingToolkit<br>model]
...
begin
	C = [1 0 0 0; 0 1 0 0] # We can measure he levels in tanks 1 and 2
	sys = ss(A,B,C,0)
end;
...
gangoffourplot(lqg, ylabel="", xlabel="", lab="", legend=:bottomright, titlefontsize=12, plot_titlefontsize=12)
...
begin
	@named d = Blocks.Step(start_time=10, height=-0.5, smooth=true)
	
	connections = [
		# TODO: add operating point to Blocks.StateSpace
		lqg_controller.output.u[1] + u0 + d.output.u ~ quadsys.u[1]
		lqg_controller.output.u[2] + u0 ~ quadsys.u[2]
		collect(lqg_controller.input.u  .~ quadsys.y .- h0[1:2])
	]

	@named closed_loop = ODESystem(connections, t, systems=[lqg_controller, quadsys, d])
end;
...
begin
	...
	plot!(res.t, (res.y .+ [h0[1:2]; 0; 0])', sp=(1:4)', lab="linear", framestyle=:zerolines)
end
